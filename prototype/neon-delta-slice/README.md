...
2. An AI can build and **observe its own work** on a game — autonomously
   ...
   [doc 24a](../../docs/24a-ai-development-and-self-observation.md).)
...
link them, every building) is built **in code** from constants in the scripts,
so the world map lives in a readable diff rather than a binary `.tscn`.
...
| File | Role |
|---|---|
| `Main.tscn` / `scripts/main.gd` | The scene root: spins up the subsystems below, owns `_ready`/`_process` and the run director. |
| `scripts/world_geo.gd` · `world_env.gd` | The world: sea, the seven districts (ground, buildings, tidal flood overlays) and causeways; sky, lighting, day/night, tide. |
| `scripts/player.gd` · `weapons.gd` | The car (heavy `VehicleBody3D`), the on-foot `CharacterBody3D`, the chase cam, enter/exit, and the raycast gun. |
| `scripts/heat.gd` · `traffic.gd` · `missions.gd` · `hud.gd` | The 0–5 Heat/pursuer system, civilian traffic & pedestrians, missions/money, and the HUD. |
| `scripts/bot.gd` | The reference "proxy player": a waypoint-following driver that tours all seven districts via the causeways. |
| `scripts/capture.gd` | `VizCapture` autoload — grabs frames during a self-check run. |
...
design. See [doc 24a](../../docs/24a-ai-development-and-self-observation.md).
